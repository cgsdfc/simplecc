...
Program *AstBuilder::visit_program(Node *node) {
...
void AstBuilder::visit_const_decl(Node *node, std::vector<Decl *> &decls) {
...
Decl *AstBuilder::visit_const_item(Node *node, BasicTypeKind type) {
...
int AstBuilder::visit_integer(Node *node) {
...
void AstBuilder::visit_declaration(Node *node, std::vector<Decl *> &decls) {
...
std::vector<Expr *> AstBuilder::visit_arglist(Node *node) {
...
Expr *AstBuilder::visit_atom_trailer(Node *node, const String &name,
...
Expr *AstBuilder::visit_atom(Node *node, ExprContextKind context) {
...
Stmt *AstBuilder::visit_write_stmt(Node *node) {
  Expr *string = nullptr;
  Expr *expr = nullptr;
...
void AstBuilder::visit_decl_trailer(Node *node, Node *type_name, Node *name,
                                    std::vector<Decl *> &decls) {
...
Stmt *AstBuilder::visit_return_stmt(Node *node) {
...
void AstBuilder::visit_stmt(Node *node, std::vector<Stmt *> &stmts) {
...
Stmt *AstBuilder::visit_flow_stmt(Node *node) {
  auto first = node->FirstChild();
...
Stmt *AstBuilder::visit_if_stmt(Node *node) {
...
Expr *AstBuilder::visit_binop(Node *node, ExprContextKind context) {
...
void AstBuilder::visit_funcdef(BasicTypeKind return_type, const String &name,
...
void AstBuilder::visit_paralist(Node *node, std::vector<Decl *> &paralist) {
...
Expr *AstBuilder::visit_factor(Node *node, ExprContextKind context) {
...
Stmt *AstBuilder::visit_stmt_trailer(Node *node, Node *name) {
  ...
  if (first->getType() == Symbol::arglist) {
    ...
    auto subscript = new Subscript(name->getValue(), index,
                                   ExprContextKind::Store, node->getLocation());
...
void AstBuilder::visit_compound_stmt(Node *node, std::vector<Decl *> &fn_decls,
                                     std::vector<Stmt *> &fn_stmts) {
...
Stmt *AstBuilder::visit_read_stmt(Node *node) {
...
Expr *AstBuilder::visit_expr(Node *node, ExprContextKind context) {
...
void AstBuilder::visit_var_decl(Node *node, std::vector<Decl *> &decls) {
  auto type_name = node->FirstChild();
  auto type = visit_type_name(type_name);
...
Decl *AstBuilder::visit_var_item(Node *node, BasicTypeKind type) {
...
Stmt *AstBuilder::visit_while_stmt(Node *node) {
...
BasicTypeKind AstBuilder::visit_type_name(Node *node) {
  return BasicTypeKindFromString(node->FirstChild()->getValue());
}

int AstBuilder::visit_subscript2(Node *node) {
...
namespace simplecc {
Program *BuildAstFromNode(const Node *node) { return AstBuilder().Build(node); }
} // namespace simplecc